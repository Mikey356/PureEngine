...
{
	std::string vertexSource = readTextFile("Resources/shaders/OpenGL/" + vertexPath);
	std::string fragmentSource = readTextFile("Resources/shaders/OpenGL/" + fragmentPath);
...
void Shader::Load(const std::string& vertexPath, const std::string& fragmentPath, const std::string& geomPath)
{
	std::string vertexSource = readTextFile("Resources/shaders/OpenGL/" + vertexPath);
	std::string fragmentSource = readTextFile("Resources/shaders/OpenGL/" + fragmentPath);
	std::string geometrySource = readTextFile("Resources/shaders/OpenGL/" + geomPath);
...
void ComputeShader::Load(const std::string& computePath)
{
...
void ComputeShader::CheckCompileErrors(GLuint shader, std::string type)
{
	...
	else
	{
		GLint success;
		GLchar infoLog[1024];
		glGetProgramiv(shader, 1024, &success);
		if (!success)
		{
			glGetProgramInfoLog(shader, 1024, NULL, infoLog);
...
void ComputeShader::SetBool(const std::string& name, bool value)
{
	...
	// glUniform1i(glGetUniformLocation(m_ID, name.c_string()), (int)value);
}
